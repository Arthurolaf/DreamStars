...
import sys
import random
from PySide6.QtCore import QDataStream, QTimer, QByteArray, QIODevice,Qt,QTimer
...
class Server_client(QDialog):
    def __init__(self, parent=None):
        ...
        self._timer.start()
        self._timer.timeout.connect(self.read_fortune)
        ...
        while not self._tcp_server.listen(QHostAddress("127.0.0.1"),self.port) and self.port != 2023:
            if not self._tcp_server.listen(QHostAddress("127.0.0.1"),self.port):
                reason = self._tcp_server.errorString()
                if "is already in use" in reason:
                    self.port += 1
                else:
        ...
        #self._get_fortune_button.clicked.connect(self._timer.start())
...
#         super().__init__(parent)
    ...
    def request_new_fortune(self):
        ...
        self._tcp_socket.connectToHost(self._host_line_edit.text(), int(self._port_line_edit.text()))
